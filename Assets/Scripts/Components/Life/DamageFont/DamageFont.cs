...
public class DamageFont : MonoBehaviour
{
    ...
    private static float lowestDamage = 0f; // 가장 작은 크기로 출력할 데미지 수치
    private static float highestDamage = 5000f; // 가장 큰 크기로 출력할 데미지 수치
    private static float minFontScale = 0.6f; // 최소 데미지 폰트 크기 (scale)
    private static float maxFontScale = 2f; // 최대 데미지 폰트 크기 (scale)
    ...
    public void Activate(float damage, bool isCritical, Vector3 target, Vector3 randomRange)
    {
        ...
        rect.transform.position =
            DamageFontManager.Instance.cam.WorldToScreenPoint(pos) + new Vector3(offset.x, offset.y, 0);
        ...
        rect.DOScale(Mathf.Lerp(minFontScale, maxFontScale,
                ((damage - lowestDamage) / (highestDamage - lowestDamage))) * (isCritical ? 1.5f : 1f), 0.4f).From(0.1f)
            .SetEase(damagePopEase);
    ...
    IEnumerator ChasePositionCo()
    {
        ...
        {
            yield return null;
            rect.transform.position = DamageFontManager.Instance.cam.WorldToScreenPoint(pos) +
